...
const MatchDetails = () => {
  const [matchDetailsItems, setMatchDetailsItems] = useState<
  ...
  const [visitorTeamId, setvisitorTeamId] = useState<number | undefined>(
    undefined
  );
      ...
      if (event.type === "substitution") {
        return (
          <li key={event.player_name}>
            ...
            <span>
              {event.player_name} for {event.related_player_name}
            </span>
          </li>
        );
      } else {
  ...
  const fetchMatchDetails = async () => {
    if (!matchDetailsItems) {
      const response = await fetch(
        `https://soccer.sportmonks.com/api/v2.0/fixtures/${location.pathname.slice(
          7
        )}?api_token=${apiKey}&include=lineup,bench,localTeam,visitorTeam,localCoach,visitorCoach,events`
      );
        ...
        <>
          <div className="match__info">
            <figure className="match__team match__team--home">
              <figcaption className="match__team-name">
                {matchDetailsItems.localTeam.data.name}
              </figcaption>
              <img
                className="match__logo  match__logo--home"
                src={matchDetailsItems.localTeam.data.logo_path}
                alt="team-logo"
              ></img>
            </figure>

            <p className="match__score">
              <span className="match__full-time">
                {matchDetailsItems.scores.ft_score}
              </span>
              <span className="match__half-time">
                {matchDetailsItems.scores.ht_score}
              </span>
            </p>

            <figure className="match__team match__team--away">
              <img
                className="match__logo match__logo--away"
                src={matchDetailsItems.visitorTeam.data.logo_path}
                alt="team-logo"
              ></img>
              <figcaption className="match__team-name">
                {matchDetailsItems.visitorTeam.data.name}
              </figcaption>
            </figure>
          </div>
          ...
          <div className="match__switch">
            <button
              onClick={() => setIsSquadInfoActive(true)}
              className={isSquadsInfoActive ? "active" : ""}
            >
              Squads
            </button>
            <button
              onClick={() => setIsSquadInfoActive(false)}
              className={isSquadsInfoActive ? "" : "active"}
            >
              Events
            </button>
          ...
          <div
            className={
              isSquadsInfoActive ? "match__squads active" : "match__squads"
            }
          >
            <div className="match__home-squad">
              ...
              <h4>Squad</h4>
              <ul>{renderSquad(matchDetailsItems.lineup.data, localTeamId)}</ul>
              <h4>Bench</h4>
              <ul>{renderSquad(matchDetailsItems.bench.data, localTeamId)}</ul>
            </div>
          ...
          <div
            className={
              isSquadsInfoActive ? "match__events" : "match__events active"
            }
          >
            <ul>{renderEvents(matchDetailsItems.events.data)}</ul>
          </div>
        </>
      )}
    </MatchDetailsWrapper>
