...
const TopScorers = () => {
  const [topScorersMode, setTopScorersMode] = useState<"goals" | "assists">(
    "goals"
  );
  const [topGoalsScorers, setTopGoalsScorers] = useState<
    TopGoalsScorers | undefined
  >(undefined);
  const [topAssistsScorers, setTopAssistsScorers] = useState<
    TopAssistsScorers | undefined
  >(undefined);
    ...
    return topGoalsScorers?.data.map((scorer) => {
      return (
        <li className="top-scorers__item" key={scorer.player.data.fullname}>
          ...
          <img
            className="top-scorers__photo"
            src={scorer.player.data.image_path}
            alt="player"
          />
          <span className="top-scorers__name">
            {scorer.player.data.fullname}
          </span>
  ...
  const fetchTopScorers = async () => {
    if (!topGoalsScorers && !topAssistsScorers) {
      const response = await fetch(
        `https://soccer.sportmonks.com/api/v2.0/topscorers/season/18369/aggregated?api_token=${apiKey}&include=aggregatedGoalscorers.player,aggregatedAssistscorers.player,aggregatedCardscorers.player`
      );
  ...
  return (
    <TopScorersWrapper>
      ...
      <div className="top-scorers__switch">
        <button
          className={topScorersMode === "goals" ? "active" : ""}
          onClick={() => setTopScorersMode("goals")}
        >
          Goals
        </button>
        <button
          className={topScorersMode === "assists" ? "active" : ""}
          onClick={() => setTopScorersMode("assists")}
        >
          Assists
        </button>
      </div>
      <ul className="top-scorers__list">
        {!topGoalsScorers && !topAssistsScorers ? (
          <li>loading...</li>
        ) : (
          renderTopScorersInit()
        )}
      </ul>
    </TopScorersWrapper>
  );
